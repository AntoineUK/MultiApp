...
//            db.execSQL("DROP TABLE IF EXISTS " + TABLE_CONTACTS);
    ...
    public void addQuestion(QuestionMultiple question) {
    ...
    public QuestionMultiple[] getAllQuestions(String diff) {
        String selectQuery = "SELECT * FROM " + TABLE_QUIZ_QUESTIONS + " WHERE difficulty=" + "'" + diff + "'" + ";";
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery(selectQuery, null);
        List<QuestionMultiple> questionList = new ArrayList<QuestionMultiple>();

        if (cursor.moveToFirst()) {
            do {
                QuestionMultiple q = new QuestionMultiple(99999, "temp", new String[5], new ArrayList<ArrayList<String>>(), new byte[5], "easy", "blank");
        ...
        db.close();
        QuestionMultiple[] x = questionList.toArray(new QuestionMultiple[questionList.size()]);
